...
function SW.UpdateLightStyle( s )
	...
	if( GetConVarNumber("sw_sv_update_lighting") == 1 and SW.LastLightStyle != s ) then
...
function SW.InitDayNight()
	...
	if( GetConVarNumber("sw_sv_update_skybox") == 1 ) then
...
function SW.InitPostEntity()
	...
	if( !table.HasValue( SW.MapBlacklist, string.lower( game.GetMap() ) ) ) then
		...
		SW.UpdateLightStyle( GetConVarString("sw_sv_light_max_night") )
		
		-- if( SW.UpdateSun and SW.Sun and SW.Sun:IsValid() ) then
		if( GetConVarNumber("sw_sv_update_sun") == 1 and SW.Sun and SW.Sun:IsValid() ) then
		...
		if( GetConVarNumber("sw_sv_update_skybox") == 1 and ( !SW.SkyPaint or !SW.SkyPaint:IsValid() ) ) then
...
function SW.GetRealTime()
	...
	return GetConVarNumber("sw_sv_realtime_offset") + tab.hour + tab.min / 60 + tab.sec / 3600
...
function SW.DayNightThink()
	...
	if( !table.HasValue( SW.MapBlacklist, string.lower( game.GetMap() ) ) ) then
		...
		if( GetConVarNumber("sw_sv_enabletime") == 0 ) then
			
			if( SW.Time >= 20 or SW.Time < 4 ) then
				
				-- SW.Time = SW.Time + FrameTime() * SW.NightTimeMul
				SW.Time = SW.Time + FrameTime() * GetConVarNumber("sw_sv_night_scale")
				
			else
				
				-- SW.Time = SW.Time + FrameTime() * SW.DayTimeMul
				SW.Time = SW.Time + FrameTime() * GetConVarNumber("sw_sv_day_scale")
				
			end
			
			-- if( SW.Realtime ) then
			if( GetConVarNumber("sw_sv_realtime") == 1 ) then
		...
		if( !SW.NextClientUpdate or CurTime() > SW.NextClientUpdate ) then
			...
			SW.NextClientUpdate = CurTime() + GetConVarNumber("sw_sv_perf_updatedelay_client")
		...
		local s = string.char( math.Round( Lerp( mul, string.byte( GetConVarString("sw_sv_light_max_night") ), string.byte( GetConVarString("sw_sv_light_max_day") ) ) ) )
		
		if( SW.WeatherMode != "" ) then
			
			-- s = string.char( math.Round( Lerp( mul, string.byte( SW.MaxDarkness ), string.byte( SW.MaxLightnessStorm ) ) ) )
			s = string.char( math.Round( Lerp( mul, string.byte( GetConVarString("sw_sv_light_max_night") ), string.byte( GetConVarString("sw_sv_light_max_storm") ) ) ) )
		...
		if( GetConVarNumber("sw_sv_update_sun") == 1 and SW.Sun and SW.Sun:IsValid() ) then
			...
			if( !SW.NextSunUpdate or CurTime() > SW.NextSunUpdate ) then
				...
				SW.NextSunUpdate = CurTime() + GetConVarNumber("sw_sv_perf_updatedelay_sun")
		...
		elseif( SW.Time < 18 ) then
			...
			if( SW.LastTimePeriod != SW_TIME_DAWN ) then
				...
				if( GetConVarNumber("sw_sv_mapoutputs") == 1 ) then
		...
		if( GetConVarNumber("sw_sv_update_skybox") == 1 and SW.SkyPaint and SW.SkyPaint:IsValid() ) then
			
			if( !SW.NextSkyUpdate or CurTime() > SW.NextSkyUpdate ) then
				
				-- SW.NextSkyUpdate = CurTime() + SW.SkyUpdateDelay
				SW.NextSkyUpdate = CurTime() + GetConVarNumber("sw_sv_perf_updatedelay_sky")
				...
				if( SW.WeatherMode != "" and !SW.GetCurrentWeather().DefaultSky ) then
					...
					SW.SkyPaint:SetStarSpeed( GetConVarNumber("sw_sv_starrotatespeed") )
...
function SW.PauseTime( b )
	...
	-- SW.TimePaused = b
	ConVar("sw_sv_enabletime"):SetBool( b )
