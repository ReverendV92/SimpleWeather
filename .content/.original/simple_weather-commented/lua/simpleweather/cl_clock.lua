...
function SW.DrawClock()
	...
	-- if( !GetConVar( "sw_cl_clock_show" ):GetBool() ) then return end
	if( GetConVarNumber( "sw_cl_clock_show" ) != 1 ) then return end
	...
	local text
	-- if( SW.Clock24 ) then
	if( GetConVarNumber("sw_cl_clock_style") == 1 ) then
		if( min < 10 ) then
			min = "0" .. min
		end
		text = hr .. ":" .. min
	...
	if GetConVarNumber("sw_cl_clock_position") == 1 then
	-- if SW.ClockTop then
