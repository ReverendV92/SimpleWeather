...
local function nRedownloadLightmaps()
	...
	render.RedownloadAllLightmaps( SW.StaticPropLighting );
...
function SW.RainSoundThink()
	...
	if( GetConVar( "sw_playsounds" ):GetInt() == 0 ) then
...
function SW.WindSoundThink()
	...
	if( GetConVar( "sw_playsounds" ):GetInt() == 0 ) then return end
...
function SW.InitPostEntity()
	...
	if( !table.HasValue( SW.MapBlacklist, string.lower( game.GetMap() ) ) ) then
		...
		if( SW.DisplayAtStart ) then
			...
			chat.AddText( Color( 255, 255, 255, 255 ), "This server is running ", Color( 76, 128, 255, 255 ), "SimpleWeather {{ script_version_name }}", Color( 255, 255, 255, 255 ), ". Type ", Color( 76, 128, 255, 255 ), SW.ChatCommand, Color( 255, 255, 255, 255 ), " to open the config menu." );
