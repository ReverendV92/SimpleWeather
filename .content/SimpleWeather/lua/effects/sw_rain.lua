		...
		for i = 1, n do
			...
			if SW.IsOutside( pos ) then
				...
				p:SetThinkFunction( function( p, pos )
					...
					if( bit.band( util.PointContents( p:GetPos() ), CONTENTS_WATER ) == CONTENTS_WATER ) or ( bit.band( util.PointContents( p:GetPos() ), CONTENTS_SOLID ) == CONTENTS_SOLID ) then
...
function EFFECT:PhysicsCollide( colData , collider )
	...
	if colData.HitObject:GetContents() == CONTENTS_SOLID then
