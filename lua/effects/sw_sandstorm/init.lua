function EFFECT:Init( data )
	...
	if( SW.Emitter2D ) then
		...
		for i = 1, GetConVarNumber("sw_blizzard_count") do
			
			-- local r = math.random( 0, SW.BlizzardRadius )
			local r = math.random( 0, GetConVarNumber("sw_blizzard_radius") )
			...
			local pos = data:GetOrigin() + Vector( math.cos( t ) * r, math.sin( t ) * r, math.Rand( -72, math.min( GetConVarNumber("sw_blizzard_height"), SW.HeightMin ) ) )
			...
			if( SW.IsOutside( pos ) ) then
				...
				p:SetDieTime( GetConVarNumber("sw_blizzard_dietime") )
				p:SetStartAlpha( 100 )
				p:SetStartSize( 1 )
				p:SetEndSize( 1 )
