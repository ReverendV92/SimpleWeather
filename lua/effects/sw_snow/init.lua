function EFFECT:Init( data )
	...
	if( SW.Emitter2D ) then
		...
		for i = 1, GetConVarNumber("sw_snow_count") do
			
			-- local r = math.random( 0, SW.SnowRadius )
			local r = math.random( 0, GetConVarNumber("sw_snow_radius") )
			...
			local pos = data:GetOrigin() + Vector( math.cos( t ) * r, math.sin( t ) * r, math.min( GetConVarNumber("sw_snow_height") , SW.HeightMin ) )
			...
			if( SW.IsOutside( pos ) ) then
				...
				p:SetDieTime( GetConVarNumber("sw_snow_dietime") )
				p:SetStartAlpha( 200 )
				p:SetStartSize( 1 )
				p:SetEndSize( 1 )
