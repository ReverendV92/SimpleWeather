function EFFECT:Init( data )
	...
	if( SW.Emitter3D ) then
		...
		for i = 1, n do
			...
			local r = math.random( 0, GetConVarNumber("sw_rain_radius") )
			local t = math.random( -180, 180 )
			local pos = data:GetOrigin() + Vector( math.cos( t ) * r, math.sin( t ) * r, math.min( GetConVarNumber("sw_rain_height"), SW.HeightMin ) )
			if( s == 1 ) then
				r = math.random( 0, GetConVarNumber("sw_storm_radius") )
				pos = data:GetOrigin() + Vector( math.cos( t ) * r, math.sin( t ) * r, math.min( GetConVarNumber("sw_storm_height"), SW.HeightMin ) )
			end
	...
	if( GetConVarNumber("sw_rain_showsmoke") == 0 ) then return end
